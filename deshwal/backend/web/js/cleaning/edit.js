$(document).ready(function () {
  ...
  const observer = new MutationObserver(function () {
    // checksavebutton();
    console.log("checksavebutton called");
  });
      ...
      function (data) {
        if (data.status === "success") {
          ...
          if (data.data.status == 4) {
            $('#status').val(5).trigger('change'); //5 is IQC require
             // Auto-save to database after 200ms
                setTimeout(function () {
                  cleaningTags = data.data.inventory_id;
            ...
            $('#tag_number').closest('.form-group').find('.help-block').addClass('fs-4') // 1.25rem
...
function checksavebutton() {
  var hasErrors = $('.help-block').filter(function () {
...
function updateInventory(inventoryId,tagNumber,binNumber){
  console.log("inventory update call"+cleaningTags);
  $.ajax({
    ...
    data: {
      ...
      status_id: 5, // because you are setting it to IQC Required
      _csrf: $("#csrfToken").val(),
  ...
  //code added by ptpatel on date 15-12-2025
  ...
  //end code added by ptpatel on date 15-12-2025