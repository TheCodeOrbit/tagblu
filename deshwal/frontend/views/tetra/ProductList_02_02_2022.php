...
foreach($Records as $Record): //print_r($Record);?>
	...
	<tr id="prod_tbl" class="prod_tbl">
		...
		<td class="<?php echo $Field->td_classname;?>">
			<?php 
			if($Field->fieldid==275 || $Field->fieldid==276){
		...
		?>
		<td class="<?php echo $Field->td_classname;?>">
		<?php echo CHtml::dropDownList($attr_name,$Multiple_Record->{$Field->fieldname},$fieldoptions,array('empty' => 'Select an Option','class'=>$fieldclass.' w-100 h5 input-border p-2 mb-0 ','disabled'=>$disabled,'id'=>$attr_id));?>
		<div class="error-container hide">All errors displayed Here</div>
