...
foreach($Records as $Record): //print_r($Record);?>
	...
	<tr id="prod_tbl" class="prod_tbl">
		...
		<td class="<?php echo $Field->td_classname;?>">
			<?php 
			echo CHtml::textField($attr_name,$Multiple_Record->{$Field->fieldname},array('id'=>$attr_id,'class'=>'input-border form-control '));
			?>
		...
		?>
		<td class="<?php echo $Field->td_classname;?>">
		<?php echo CHtml::dropDownList($attr_name,$Multiple_Record->{$Field->fieldname},$fieldoptions,array('empty' => 'Select an Option','class'=>'h5 input-border p-2 mb-0','id'=>$attr_id));?>
		<?php //echo $form->error($model,$Field['fieldname'], array('class'=>'ajxwarning errorMessage')); ?>
		</td>
