        ...
        foreach ($ColumnList->blocks as $BlockKey => $Block) {
            ...
            if (
                ...
                <details class="c-faqs__item">
                    ...
                    <!-- code added by ptpatel to add icon in document tab -->
                         <?php if($TabId == 23 && $icon_count == 0){
                            $icon_count++;
                                $docrecords = \app\models\Documents::find()
                                    ->where(['docid' => $Recordid])
                                    ->one();
                                if(!empty($docrecords)){
                                    $records = \app\models\Attachments::find()
                                    ->where(['attachmentsid' => $docrecords->filename])
                                    ->one();
                                    ...
                                    echo '<img src="'.$iconPath.'" class="file-icon-img" alt="'.$ext.'" file>';
                    ...
                    <?= $Block->blocklabel ?> <i class="fa-solid fa-angle-down"></i>
                     ...
                     <div class="details-container" id="details-container">
                        ...
                        ?>
                            ...
                            if ($field["uitype"] == 12 || $field["uitype"] == 27 || $field["uitype"] == 28) {
                                ...
                                if ($Record->{$field["columnname"]}) {
                                    ...
                                    if (!empty($Record->{$field["columnname"]})) {

                                        // Case 1: profile picture (image stored as path)
                                        if ($field["columnname"] == 'profilepic') {

                                            $imgUrl = $baseUrl . $Record->{$field["columnname"]};
                                        ...
                                        // Case 2: attachment stored in attachments table
                                        else {
                                            ...
                                            if ($records) {
                                                ...
                                                $iconPath = Yii::$app->HomeUrl.'thememain/img/file-icon/'.$iconClass;
                                                $hoverThumb = in_array($ext, ['jpg','jpeg','png','svg','webp']) ? `<img src='$fileUrl' class="file-hover-thumb">` : '';
                                                $Record->{$field["columnname"]} = "
                                                    <div class='file-preview-wrapper' style='display:inline-flex;align-items:center;gap:6px;'>
                                                        <img src=".$iconPath." class='file-icon-img' alt='$ext' file'>$hoverThumb
                                                        {$fileName}
                        ...
                        if ($TabId == 18 && $Block->blocklabel == 150) {
