...
class DefaultController extends ModuleController
{
    ...
    public $ModuleName='drilling';
    public $FieldId='drilling_id';
    public $TableName='drilling';
    public $TabLabel='Drilling';
    public $TabId='3';
    ...
    public function actionGetlocationddress()
    {
        ...
        if(!empty($sourcing_deal)){
            ...
            if(!empty($serviceDetailData)){
                ...
                $command = $connection
                ->createCommand("SELECT * FROM servicedetail_details WHERE service_type = 2 and servicedetail_id = :servicedetail_id and service_to_location=:service_to_location and deleted=0")
    ...
    {   
        ...
        if (!empty($columns)) {
            ...
            if(!empty($serviceDetailData)){
                ...
                $columns["servicedetail_id"] = $servicedetail_id;
                $command = $connection
                ->createCommand("SELECT * FROM servicedetail_details WHERE service_type = 2 and servicedetail_id = :servicedetail_id and deleted=0")
                ->bindValue(":servicedetail_id", $servicedetail_id);
                $serviceDetailItems = $command->queryAll();
                foreach($serviceDetailItems as $key=>$val){
    ...
    public function actionDrillingcompleted(){
        try{
            $transaction_enabled = false;
            if (!(isset($_POST['drilling_completed']) && $_POST["drilling_completed"] == "Yes")) {
            ...
            $oldAttributes = Yii::$app->db->createCommand("select * from `drilling` where drilling_id=:drilling_id")
                ->bindValue(":drilling_id", $Record)
                ->queryOne();
            if (empty($oldAttributes)) {
                throw new Exception("No record found");
            }
            $current_drilling_status = $oldAttributes["drilling_status"];
            ...
            if($current_drilling_status != 4){
                throw new Exception("Invalid action, current status is not 'Drilling in process/ Activity in process'");
            }
            if($fe_user != $id){
            ...
            $assets_query = DrillingAssetDetails::find()->where(['drilling_id' => $Record, 'deleted' => 0]);
            ...
            if ($assets_count > 0) {
                ...
                foreach ($assets_data as $ad) {
                    ...
                    if (empty($ad->drilling_date)) {
                        throw new Exception("'Drilling Date' is required for all records in the 'ASSET DETAILS' section");
                    }
                    if (empty($ad->drilling_completed )) {
                        throw new Exception("'Drilling Completed' status is required for all records in the 'ASSET DETAILS' section");
                    }
                    if ($ad->drilling_completed  != 1) {
                        throw new Exception("'Drilling Completed' status has to be 'Yes' for all records in the 'ASSET DETAILS' section");
                    }
                }
            } else {
            ...
            $new_drilling_status = 5;
            $newattributes = array("drilling_status" => $new_drilling_status, "completed_date" => date("Y-m-d"));

            $sql = "UPDATE drilling set drilling_status = :drilling_status,completed_date=:completed_date,modifiedtime = :modifiedtime,modifiedby = :modifiedby where drilling_id = :id";
            $status = Yii::$app->db->createCommand($sql)
                ->bindValue(":drilling_status", $new_drilling_status)
    ...
    public function actionDrillingassets(){
        try{
            $transaction_enabled = false;
            if (!(isset($_POST['drilling_asset_details']) && $_POST["drilling_asset_details"] == "Yes")) {
                throw new Exception("Invalid Action");
            }
            $role = Yii::$app->user->identity->role ?? null;
		    $id = Yii::$app->user->id;
    ...
    public function actionDrillingclientsign(){
        try{
            $transaction_enabled = false;
            ...
            $hdd_completed = (int)$oldAttributes["hdd_completed"];
            $client_sign = $oldAttributes["image"];
            $drilling_no = $oldAttributes["drilling_no"];
            $random_number = rand(10,99);
            $signature_file_name = "$drilling_no^client_signature_".$random_number.".png";
            if(empty($hdd_count)){
                throw new Exception("Invalid action ! HDD Count is not found");
            }
            if($current_drilling_status != 4){
            ...
            $result = EditModel::saveGeneratedFiles($image_data[1], $signature_file_name,$Record,'drilling',$oldAttributes, $new_attributes,"image/png");
            if (!$result['success']) {
            ...
            $sql = "UPDATE drilling set image = :image,modifiedtime = :modifiedtime,modifiedby = :modifiedby where drilling_id = :id";
            $status = Yii::$app->db->createCommand($sql)
    ...
    public function actionGetsourcingdetail(){
        ...
        $command = $connection->createCommand("
                         ...
                         WHERE sourcingdeal_id = :sourcingdeal_id and service_type=2
