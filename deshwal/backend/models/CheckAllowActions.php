...
class CheckAllowActions
{
	...
	function checkAllowedActions($TabId, $ModuleName, $Recordid, $Record, $ExportPermission = null, $ImportPermission = null)
	{
		...
		if ($TabId == 8) {
			//added on 19 dec 2025 for making auto approval
			//first get the action_label
			$rule = ApprovalRule::find()
			->where(['module' => $ModuleName])
			->andWhere(['!=', 'action_label', ''])
			->orderBy('id ASC')
			->one();
			 if (!$rule) {
                // throw new \Exception("No approval rule found for module: {$ModuleName}");
				 return $actions;
            }
			$stage_column = $rule->stage_column;
			$rule2 = ApprovalRule::find()
			->where(['module' => $ModuleName,'source_stage'=>$Record[$stage_column]])
			// ->andWhere(['!=', 'action_label', ''])
			->orderBy('id ASC')
			->one();
			 if (!$rule2) {
               // throw new \Exception("No rule found for module + stage.");
			   // return $actions;
				$source_role = '';

            }
			else
			$source_role = $rule2->source_role;
			

			//check if solution team profile then show buttons to add products

			//check team responsible
			if ($Record[$stage_column] == '2' && ($user_role == $source_role)) {
				$team_responsible = explode(",", $Record['team_responsible']);
				if (!empty($team_responsible)) {
					foreach ($team_responsible as $tm) {
						// if ($tm == '1' || $tm == '3')//solution team
						if ($tm == '1')// as per New CR changes on date 17-02-2026
						{
						...
						if ($tm == '3')// as per New CR changes on date 17-02-2026
						{
							if (!empty($Record['sf_assigned']))
								$actions["ApproveOpportuntiy"] = true;
							else {
								$actions["ApproveOpportuntiy"] = false;
								break;
			...
			// || $Record['opportunity_stage'] == '6' || $Record['opportunity_stage'] == '7'
			// if ($Record['opportunity_stage'] == '4') {
			if ($Record['opportunity_stage'] == '4' || $Record['opportunity_stage'] == '5' ) {
				//check team responsible
				$team_responsible = explode(",", $Record['team_responsible']);
				if (!empty($team_responsible)) {
					foreach ($team_responsible as $tm) {
						...
						// {
						if ($tm == '1' && !empty($Record['sa_assigned'])) {

							$members = array_map('trim', explode(',', $Record['sa_assigned']));
						...
						// {
						if (($tm == '1' || $tm == '3') && !empty($Record['sf_assigned'])) {

							$members = array_map('trim', explode(',', $Record['sf_assigned']));
							...
							// if ($cnt_s == 0) {
							// added on 14 feb 2026 by deepika to allow edit after pricing done also
							if ($cnt_s == 0 || $cnt_s > 0) {
								$actions["EditOpportunity"] = true;
								break;
							}
						...
						// {
						if ($tm == '2' && !empty($Record['procurement_team_member'])) {

							$members = array_map('trim', explode(',', $Record['procurement_team_member']));
		...
		if($TabId == 74)
		{
			...
			$source_role = $rule2->source_role;
			// echo $source_role;die;
			if($source_role == $user_role)
			{
				$approve_status = $rule2->approve_status;
				$reject_status = $rule2->reject_status;
				$modify_status = $rule2->modify_status;
				if(!empty($approve_status) && $approve_status != null)
					$actions['showapprove'] = true;
				if(!empty($reject_status) && $reject_status != null)
					$actions['showreject'] = true;
				if(!empty($reject_status) && $reject_status != null)
					$actions['showmodify'] = true;
		...
		if($TabId == 51)//sorucing deal auto approval
		{
			//first get the action_label
			$rule = ApprovalRule::find()
			->where(['module' => $ModuleName])
		...
		if ($TabId == 32) { // grn
			$detail = \app\models\Detaileditsetting::find()
				->where([
					'tabid'       => $TabId,
			...
			$recordStageValue = null;
			if ($stageField !== '' && strcasecmp($stageField, 'NA') !== 0) {
				$recordStageValue = $Record[$stageField] ?? null;

				if ($stageValue !== null && $stageValue !== '' && (string)$recordStageValue !== (string)$stageValue) {
					return $actions;
				}
			}
