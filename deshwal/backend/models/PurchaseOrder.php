...
class PurchaseOrder extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['total_amount', 'basic_cp', 'total_cgst_amount', 'total_sgst_amount', 'total_igst_amount', 'total_amount1', 'margin', 'margin_percentage', 'tcs_percentage', 'tcs_amount', 'grand_total'], 'number'],
            ...
            [['sourcing_deal_name'], 'string', 'max' => 200],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'sourcing_deal_name' =>'Sourcing Deal Name',
            'round_off'=>'Round Off'
        ];
    }
     ...
     public function validateDataIntegrityForPoApproval($stage)
    {
            ...
            foreach ($purchase_order_itemsdetail as $item) {
                ...
                if (!isset($item['cost_price']) || $item['cost_price'] === '') {
            ...
            // if((float)$total_amount_of_items != (float)$basic_cp){
            ...
            // }
            if (round((float)$total_amount_of_items, 2) !== round((float)$basic_cp, 2)) {
                throw new Exception(
                    "Total Amount of Purchase Order $total_amount_of_items do not match with the Basic CP $basic_cp of all Items. Please check the submitted data"
                );
            }
  ...
  /**
   * Added functionalithy For stage = 3 (Approve) and No payments against it
   *  so display the edit btn only to super admin
   * @Date 13/11/2025Ḍ
   * @param mixed $record
   * @param mixed $user
   * @return bool|null
   */
  public function showEditBtnPO($record = [],$user = [])
    {
        if (isset($user) && $user->is_super_admin) {
            $payments = Payments::find()->where(['po' => $record['purchase_order_id']])
    ...
    public static function handleRelatedModuleStageChange($Record, $status)
    {
        //save to vp reports for purchase order
            $modelleadetail = new PurchaseOrder();
            $modelleadetail->saveToVpReports($Record);
    }
