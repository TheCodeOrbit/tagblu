...
use yii\helpers\Html;
	...
	$user_id=Yii::$app->user->id;//$_SESSION[$siteDir."_id"];
	...
	$ActionUrl= Url::to([$ModuleName], true) . '/';
?>		
<?php 
...
<div id="ListDrill" class="show">
	<div class="body-menu d-flex justify-content-between">
		...
		echo Html::beginForm($url, 'post', ['name' => 'listsearchfm', 'id' => 'listsearchfm']); ?>
			...
			<div class="d-flex transform-center">
				<?php 
				if($ModuleName=="manpower" || $ModuleName=="equipment" || $ModuleName=="contractormaster" || $ModuleName=="location" || $ModuleName=="material"){
				}else{?>
				...
				<table class="table-view table table-striped">
					<thead>
						<tr class="list-general table-primary">
							...
							<th>Action</th>
							<?php } ?>
							<?php if($ModuleName=="manpower" || $ModuleName=="ehs" || $ModuleName=="debtors_plan" || $ModuleName=="water_management" || $ModuleName=="revenue_data"  || $ModuleName=="log_user_details"  || $ModuleName=="production" || $ModuleName=="users"  || $ModuleName=="equipment" || $ModuleName=="contractormaster" || $ModuleName=="location" || $ModuleName=="material" || $ModuleName=="other_cost" || $ModuleName=="rake_plan"){
							}else{?>
								<th>Status</th>
							<?php }?>
						...
						//echo CHtml::beginForm($url, 'POST',array("name"=>"listsearchfm_master","id"=>"listsearchfm_master")); ?>
							<input type="hidden" value="<?php //echo $listserchvals['startdate']; ?>" name="startdate" id="startdate" /> 
							<input type="hidden" value="<?php //echo $listserchvals['enddate']; ?>" name="enddate" id="enddate" /> 
							<?php if($ModuleName=="manpower" || $ModuleName=="equipment" || $ModuleName=="contractormaster" || $ModuleName=="location" || $ModuleName=="material"){?>
								<tr>
									<?php //echo $listserchvals['vals']; ?>
									<th>
										...
										<a href="<?php echo $ActionUrl;?>Detail/Record/<?php echo base64_encode( $Record['RecordId']); ?>">
											<div class="action-icon-container d-flex justify-content-center align-items-center">
												...
												<svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="action-icon">
													<path d="M17.02 19.5C17.85 19.5 18.5228 18.8284 18.5228 18C18.5228 17.1716 17.85 16.5 17.02 16.5C16.1899 16.5 15.5171 17.1716 15.5171 18C15.5171 18.8284 16.1899 19.5 17.02 19.5Z" fill="var(--color-primary)"/>
													<path d="M22.8639 17.6093C22.3996 16.4287 21.5984 15.41 20.5596 14.6796C19.5208 13.9491 18.2901 13.539 17.02 13.5C15.7499 13.539 14.5192 13.9491 13.4804 14.6796C12.4416 15.41 11.6404 16.4287 11.1761 17.6093L11.0085 18L11.1761 18.3907C11.6404 19.5713 12.4416 20.59 13.4804 21.3204C14.5192 22.0509 15.7499 22.461 17.02 22.5C18.2901 22.461 19.5208 22.0509 20.5596 21.3204C21.5984 20.59 22.3996 19.5713 22.8639 18.3907L23.0314 18L22.8639 17.6093ZM17.02 21C16.4255 21 15.8444 20.8241 15.3501 20.4944C14.8558 20.1648 14.4706 19.6962 14.2431 19.148C14.0156 18.5999 13.956 17.9967 14.072 17.4147C14.188 16.8328 14.4743 16.2982 14.8946 15.8787C15.315 15.4591 15.8506 15.1734 16.4336 15.0576C17.0167 14.9419 17.621 15.0013 18.1702 15.2284C18.7195 15.4554 19.1889 15.8399 19.5192 16.3333C19.8494 16.8266 20.0257 17.4067 20.0257 18C20.0247 18.7953 19.7077 19.5578 19.1443 20.1202C18.5808 20.6826 17.8169 20.999 17.02 21Z" fill="var(--color-primary)"/>
													<path d="M5.74854 12.75H9.50569V14.25H5.74854V12.75Z" fill="var(--color-primary)"/>
													<path d="M5.74854 9H14.7657V10.5H5.74854V9Z" fill="var(--color-primary)"/>
													<path d="M5.74854 5.25H14.7657V6.75H5.74854V5.25Z" fill="var(--color-primary)"/>
													<path d="M17.0201 1.5H3.49432C3.0961 1.50119 2.71453 1.6596 2.43295 1.94065C2.15136 2.2217 1.99264 2.60254 1.99146 3V21C1.99264 21.3975 2.15136 21.7783 2.43295 22.0593C2.71453 22.3404 3.0961 22.4988 3.49432 22.5H9.50577V21H3.49432V3H17.0201V11.25H18.5229V3C18.5218 2.60254 18.363 2.2217 18.0815 1.94065C17.7999 1.6596 17.4183 1.50119 17.0201 1.5Z" fill="var(--color-primary)"/>
												</svg>
											</div>
										</a>
									<?php }else{ ?>		
								...
								<td>
									...
									<?php } ?>
										<?php if(in_array('2',$val) and $module == $permod) {?>
											...
											if($profilename=="datachamp"){ 

												//echo "deepika ".$datadate." <= ".$todaydate." datachamplimit=".$datachamplimit." approve status=".$Record['approve_status'] ;
												//commented on 05jan24, edit was not working by deepika
											//if($datadate<=$todaydate) && $datadate>=$datachamplimit && $Record['approve_status']!=1){
												//added on 05jan24, edit was not working by deepika
											if(strtotime($datadate)<=strtotime($todaydate) && strtotime($datadate)>=strtotime($datachamplimit) && $Record['approve_status']!=1){
												?>
												<a href="<?php echo $ActionUrl;?>Edit/Record/<?php echo base64_encode( $Record['RecordId']); ?>"> 
										<div class="action-icon-container d-flex justify-content-center align-items-center">
											<div class="action-icon-container-label">
									...
									<?php } ?>
										...
										<div class="action-icon-container d-flex justify-content-center align-items-center">
											...
											<?php }}else{?>
												...
												<?php }}
											} else if ($user_id=='1' ||$user_id=='222' ||$user_id=='217'||$user_id=='208') { ?> 
													<a href="<?php echo $ActionUrl;?>Edit/Record/<?php echo base64_encode( $Record['RecordId']); ?>">
														<div class="action-icon-container d-flex justify-content-center align-items-center">
															<div class="action-icon-container-label">
																<span>Edit</span>
