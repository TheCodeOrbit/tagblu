    ...
    foreach ($relatemodules as $key => $value) {
                    ...
                    if (!empty($showfield)) {
                        ...
                        if (!empty($value['related_columns'])) {
                            ...
                            foreach ($colsArray as $cname) {
                                $dynamicLabels[$cname] = $col_label_map[$cname]['fieldlabel'] ?? '';
                            }
                        ...
                        $rows = Yii::$app->db->createCommand($sql_h)
                            ->bindValue(":record", $Recordid)
                            ->queryAll();
                        ...
                        if (!empty($rows)) {
                            ...
                            foreach ($rows as $head) {
                                ...
                                // pkey for this record
                                $single_pkey = $head[$pkey] ?? null;

                                // -----------------------------
                                // SHOWFIELD value logic
                                // -----------------------------
                                if ($showfield == 'contacts_id') {
                                    $sql_contact = "
                                        SELECT CONCAT(first_name,' ', IF(last_name IS NULL,'',last_name)) AS fullname
                                        FROM contacts 
                                        WHERE contacts_id = :record 
                                        AND deleted = 0
                                        LIMIT 1
                                    ";
                                    ...
                                    $single_showfieldval = $headval['fullname'] ?? '';

                                } else {
                                    $single_showfieldval = $head[$showfield] ?? '';
                                }
        ...
        <div class="collapse-container">
            ...
            <label class="collapse-header" for="toggle-<?= ucfirst($value['modulename']); ?>">
                <div class="icons-coll <?= ucfirst($value['modulename']); ?>"
     style="width:50px; height:50px; display:flex; align-items:center; justify-content:center; flex-shrink:0;"
     title="<?= ucfirst($value['modulename']); ?> Icon">
    <?= SvgRenderHelper::renderIcon($value['modulename'] . '.svg', true); ?>
</div>
                <?= $modulelabel ?> (<?= $cnt; ?>)
                <?php
                if ($relateid != 53 && $relateid != 58) {
                    ...
                    if ($relateid == 42) { //42 quote in right sidebar
                        $stage_value = Sourcingdeal::find()->select("stage")->where(['sourcingdeal_id' => $Recordid])->andWhere(['deleted' => 0])->scalar();
                        if (!empty($stage_value) && $stage_value == 10) { //10 for priceing done
                            ?>
                            <a
                    ...
                    } else if ($relateid == 72) //DEvit Quotes in right sidebar
                    {
                        ...
                        $quote_value = QuotesDit::find()->select("quote_stage")->where(['opportunity_name' => $Recordid])->andWhere(['deleted' => 0])->one();
                        $opportunity = Opportunity::find()->select("opportunity_stage")->where(['opportunity_id' => $Recordid])->andWhere(['deleted' => 0])->one();
                        // echo $relateid;
                    ...
                    {
                        // if in Quote devit Module stage is "Approved"
                        $so_value = SalesorderDit::find()->select("so_stage")->where(['deal_name' => $Recordid])->andWhere(['deleted' => 0])->one();
                        $opportunity = Opportunity::find()->select("opportunity_stage")->where(['opportunity_id' => $Recordid])->andWhere(['deleted' => 0])->one();
                        // echo $relateid;
                    ...
                    //$relateid == 78 code added by ptpatel on date 31-03-2026 when SO(DevIt) is approved then only PO can create from + btn
                    else if ($relateid == 78) //DEvit purchase order in right sidebar in sales order
                    {
                        // if in Quote devit Module stage is "Approved"
                        $so_value = SalesorderDit::find()->select("so_stage")->where(['salesorder_dit_id' => $Recordid])->andWhere(['deleted' => 0])->one();
                        // echo $relateid;
                        // print_r($so_value);die;
                        // echo $so_value->so_stage;die;
                        if ((!empty($so_value) && $so_value->so_stage == '4')) { //if SO is approved
                            ?>
                                    <a
                                        href="<?= $baseUrl; ?><?= $value['modulename']; ?>/create?sourcemodule=<?= $TabId; ?>&sourceid=<?= $Recordid; ?>">
                                        <i class="fa-solid fa-plus" title="add new"></i>
                    ...
                    } //$relateid == 78 code end added by ptpatel on date 31-03-2026 when SO(DevIt) is approved then only PO can create from + btn
                        else if ($relateid == 13) //purchase oreder in right sidebar
                    {
                        ...
                        $quote_value = Quotes::find()->select("quote_stage,account_name")->where(['quotes_id' => $Recordid])->andWhere(['deleted' => 0])->one();
                        // if(!empty($quote_value)){//this condition added by ptpatel on date 16-01-2026 when quote not found or delete = 0
                            $active_account = VendorAccount::find()->where(["vendoraccid" => $quote_value->account_name])->andWhere(['deleted' => 0])->one();
                            // !empty($active_account) && condition added by ptpatel on date 16-01-2026
                            if (!empty($active_account) && $quote_value->quote_stage == 1 && $active_account->acc_status == 2) { //1 if for quote approved 2 is for account status active
                                ?>
                                            <a
                                                href="<?= $baseUrl; ?><?= $value['modulename']; ?>/create?sourcemodule=<?= $TabId; ?>&sourceid=<?= $Recordid; ?>">
                                                <i class="fa-solid fa-plus" title="add new"></i>
                    ...
                    // on date 26-06-25
                         ...
                         $active_account = VendorAccount::find()->where(["vendoraccid" => $Recordid])->andWhere(['deleted' => 0])->one();
                        // if(!empty($active_account)){ // added by ptpatel on date 16-01-2026
                    ...
                    else if ($relateid == 55) //sourcing deal
                    {
                       ...
                       //added by deepika on 18 june 2025
                        ...
                        $services = Servicedetail::find()->select("*")->where(['related_to_id' => $Recordid])->andWhere(['deleted' => 0])->all();
                        // echo "<pre>";print_r($services);
                        // if(!empty($services)){// added by ptpatel on date 16-01-2026
                        ...
                        //below if condition added to restrict add product detail more than once by ptpatel on date 16-03-2026
                        if($relateid == 31){
                             if($cnt == 0) {
                            ?>
                                                                <a
                        ...
                        //end code for if condition added to restrict add product detail more than once by ptpatel on date 16-03-2026
