$(document).ready(function () {
  ...
  function fetchOpportunityDetails() {
    data = { opportunity: $("#opportunit_name1").val(), _csrf: $('#csrfToken').val() };
      ...
      $.ajax({
        ...
        success: function (response) {
            ...
            if (response && response.data) {
                ...
                $("#bill_to_location").val(response.data.bill_location);
                $("#state").val(response.data.bill_state);
                $("#pincode").val(response.data.bill_pincode);
                $("#gstin_no_uin").val(response.data.bill_gstin_no);
                $("#city").val("");
            } else {
  ...
  function billable_effect() {
    var option = $("#billable").val();
    if (option == 2) {
      $(".blockrow139").parent(".row").show()
      if (!$("#bill_to_location").val()) {
        fetchOpportunityDetails()
      }
    } else {
      $(".blockrow139").parent(".row").hide()
      $(".blockrow139").find("input, select, textarea").each(function() {
          if ($(this).is("select")) {
              if ($(this).hasClass("select2-hidden-accessible")) {
                  $(this).val(null).trigger("change"); // Clear Select2 dropdown
              } else {
                  $(this).prop("selectedIndex", 0); // Reset normal select to first option
              }
          } else if ($(this).is(":checkbox") || $(this).is(":radio")) {
              $(this).prop("checked", false); // Uncheck checkboxes and radio buttons
          } else {
              $(this).val(""); // Clear text inputs, number fields, etc.
          }
      });
    }
  }
  function detailViewToggle() {
    var billable = $(".detail-billable").find("span").text();
    if (billable != "Yes") {
      $(".detail-customer_name").parents(".c-faqs__item").remove();
    }
  }
  function manageProvisionToExtendTiming() {
    var extend_time_provision = $("#extend_time_provision").val();
    if (extend_time_provision == 1) {
      $("#extension_provision").prop("disabled", false).removeClass("readonly")
    } else {
      $("#extension_provision").val(null).prop("disabled", true).addClass("readonly").trigger("change");
    }
  }
  ...
  function manageServiceLift() {
    ...
    if (service_lift == 1) {
      $("#lift_timings").prop("disabled", false).removeClass("readonly")
      $("#stairs_area").val(null).prop("disabled", true).addClass("readonly").trigger("change");
    } else if (service_lift == 2) {
      $("#lift_timings").val(null).prop("disabled", true).addClass("readonly").trigger("change");
      $("#stairs_area").prop("disabled", false).removeClass("readonly")
    } else {
      $("#lift_timings").val(null).prop("disabled", true).addClass("readonly").trigger("change");
      $("#stairs_area").val(null).prop("disabled", true).addClass("readonly").trigger("change");
    }
  ...
  function fetchActivitySpocDetails() {
    data = { spoc: $("#activity_spoc1").val(), _csrf: $('#csrfToken').val() };
    $("#loading-overlay").css("display", "grid");
    $("#activity_spoc_email").val("");
    $("#activity_spoc_mobile").val("");
    $.ajax({
      type: 'POST',
      url: "getspoc",
      data: data,
      success: function (response) {
          $("#loading-overlay").css("display", "none");
          if (response && response.data) {
              $("#activity_spoc_email").val(response.data.spoc_email);
              $("#activity_spoc_mobile").val(response.data.spoc_mobile);
          } else {
              console.log("Invalid response format or missing data");
          }
      },
  ...
  function manageMachineMovement() {
    var machine_movement = $("#machine_movement").val();
    if (machine_movement == 2) {
      $(".blockrow2622").parents(".titlerow").show()
      $(".blockrow2623").parents(".titlerow").show()
    } else {
      $(".blockrow2622").parents(".titlerow").hide()
      $(".blockrow2622,.blockrow2623").find("input, select").each(function () {
        if ($(this).is("select")) {
      ...
      $(".blockrow2623").parents(".titlerow").hide()
    }
  ...
  $(document).on("change", "#machine_movement", function () {
    manageMachineMovement();
  })
  $(document).on("change", "#delivery_location_type", function () {
    manageDeliveryLocation();
  })
  ...
  $(document).on("click", ".degaussing-completed", function () {
        let data = {
            Recordid: $("#Recordid").val(),
            _csrf: $("#csrfToken").val(),
            degaussing_completed: "Yes",
        };
        $("#loading-overlay").css('display', 'grid');
        $.ajax({
            type: "POST",
            url: "degaussingcompleted",
            data: data,
            success: function (data) {
                if (data.status === "success") {
                  $(".degaussing-completed,.add-lead-btn2").remove()
  ...
  $(document).on("click", ".degaussing-client-sign", function () {
    let data = {
        Recordid: $("#Recordid").val(),
        _csrf: $("#csrfToken").val(),
        degaussing_asset_details: "Yes",
    };
    $("#loading-overlay").css('display', 'grid');
    $("#detailViewGeneralLabel").text("Degaussing Assets")
        $.ajax({
            type: "POST",
            url: "degaussingassets",
            data: data,
          success: function (data) {
              $("#loading-overlay").css('display', 'none');
                if (data.status === "success") {
  ...
  $(document).on("click", "#detail-view-general-submit", function (e) {
    ...
    if (signatureData) {
      ...
      $("#loading-overlay").css('display', 'grid');
      $.ajax({
          url: 'degaussingclientsign',
          type: 'POST',
          data: data,
          success: function (data) {
              if (data.status === "success") {
                $(".degaussing-client-sign,.add-lead-btn2").remove()
                $("#detail-view-general-info").modal("hide")
                showSuccessMessage(data.message || "Updated successfully")
                location.reload();
              } else {
  ...
  var targetNodeActivitySpoc = document.getElementById('activity_spoc1');
  if (targetNodeActivitySpoc) {
    ...
    observerActivitySpoc.observe(targetNodeActivitySpoc, { attributes: true });
  }
  ...
  if (targetNodePickupLocationInternal) {
  ...
  if (targetNodeDeliveryLocationInternal) {
    var observerDeliveryLocationInternal = new MutationObserver(function (mutationsList) {
      for (var mutation of mutationsList) {
        $("#delivery_address,#delivery_state,#delivery_city,#delivery_pin").val("");
        if (mutation.type === 'attributes' && mutation.attributeName === 'value') {
  ...
  if (targetNodeDeliveryLocationClient) {
