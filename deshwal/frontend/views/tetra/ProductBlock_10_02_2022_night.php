...
foreach($Block->Fields as $key=> $Field):
	...
	<?php 	if(strpos($Field->classname, 'ReadOnly') !== false){
			...
			echo CHtml::textField($attr_name, '',array('id'=>$attr_id, 'class'=>$Field->classname.' input-border form-control b','readonly'=>true));
				...
				//print_r($Field);
				// echo $Field->fieldid;
				if($Field->fieldid==1899 || $Field->fieldid==280 || $Field->fieldid==1806 || $Field->fieldid==2075  || $Field->fieldid==2081){
			...
			echo CHtml::textField($attr_name, '',array('id'=>$attr_id, 'class'=>$timeformat.' input-border form-control '.$attr_class."_ehs", 'placeholder'=>$p_holder));
			}
		}
