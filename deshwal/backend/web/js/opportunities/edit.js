  ...
  $(document).ready(function () {
    ...
    if (pricing_done) {
      $.getJSON(
          ...
          function (response) {
              console.log("checkpricingdone", response);

              if (response.canSubmit) {
                  // allowed → enable checkbox
                  $("#pricing_done").prop("disabled", false);
              } else {
                  // not allowed or already done → disable and check
                   $("#pricing_done")
                    // .prop("checked", true)
                    .prop("disabled", true);
              }
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response && response.data) {
        ...
        $("#bill_state_code").val(response.data.state_code).trigger("change");
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response && response.data) {
        ...
        $("#bill_from_state_code").val(response.data.statecode).trigger("change");
        $("#productTable43 tbody tr").each(function () {
            var trid = this.id;
            applyGstSplit(trid);
        });
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response && response.data) {
        ...
        // $("#billing_city").val(response.data.bill_city);
        $("#bill_from_state").val(response.data.state);
        $("#bill_from_state_code").val(response.data.state_code).trigger('change');
        $("#productTable43 tbody tr").each(function () {
            var trid = this.id;
            applyGstSplit(trid);
        });
...
$(document).on("change", "#bill_state_code, #bill_from_state_code,#warehouse_loc_business_entity", function () { 
    $("#productTable43 tbody tr").each(function () { 
        var trid = this.id;
        applyGstSplit(trid);
        settotal();
...
//function added by ptpatel to resolve CR point 1 on date 13-02-2026
...
function setsasf() {
    ...
    var team_responsible = $('#team_responsible').val() || [];

    // First reset everything to Optional
    makeOptional("sa_assigned");
    makeOptional("sf_assigned");
    makeOptional("procurement_team_member");

    // If Solution Team selected (1)
    if (team_responsible.includes('1')) {
        makeMandatory("sa_assigned");
        makeMandatory("sf_assigned");
    }

    // If Procurement Team selected (2)
    if (team_responsible.includes('2')) {
        makeMandatory("procurement_team_member");
    }

    // If Solution Factory selected (3)
    if (team_responsible.includes('3')) {
        makeMandatory("sf_assigned");
    }
...
function makeMandatory(fieldId) {
    $("#" + fieldId).removeClass("V~O").addClass("V~M");
}

function makeOptional(fieldId) {
    $("#" + fieldId).removeClass("V~M").addClass("V~O");
    clearFieldError(fieldId);
}

function clearErrorsIfFilled() {
    ...
    // }
     ...
     const fields = ["sa_assigned", "sf_assigned", "procurement_team_member"];
    fields.forEach(function(fieldId) {
        const $hidden = $("#" + fieldId + "1");
        const $field = $("#" + fieldId);
        const value = ($hidden.val() || "")
            ...
            .join(",");
        if (value !== "") {
            $field.removeClass("V~M");
            clearAssignedError(fieldId);
        }
    });
}

//end code added for CR Point -1 
...
function toggleBlocks(status) {
  ...
  switch (status) {
    ...
    case 8: // In delivery
      mandatoryByStatus = ["po_received_date"];
      break;
...
function togglePOSections() {
  ...
  let section_to_show = ['.section-customer_po_date', '.section-customer_po_num','.section-po_received_date'];
  ...
  {
    $(section_to_show.join(",")).hide();
    let $sections = $(section_to_show.join(","));
    ...
    $sections.find("input, select, textarea").each(function () {
        ...
        // Clear validation errors (Yii2)
        $sections.find('.is-invalid').removeClass('is-invalid');
        $sections.find('.help-block').text('');
  }
}
...
//code added for CR Point -1 on date 13-02-2026 by ptpatel 
...
/* Show error */
function showAssignedError(fieldId) {
    const $displayField = $("#" + fieldId);
    const $formGroup = $displayField.closest(".form-group");
    let $helpBlock = $formGroup.find(".help-block").first();

    if ($helpBlock.length === 0) {
        $helpBlock = $displayField.closest("div").find(".help-block").first();
    }

    $displayField.addClass("error");
    $formGroup.addClass("has-error");
    $helpBlock.text("This field is mandatory.");
}

/* Clear error */
function clearAssignedError(fieldId) {
    const $displayField = $("#" + fieldId);
    const $formGroup = $displayField.closest(".form-group");
    let $helpBlock = $formGroup.find(".help-block").first();
    ...
    $displayField.removeClass("error");
    $formGroup.removeClass("has-error");
    $helpBlock.text("");
}

/* Validate single assigned field */
function validateAllAssignedFields() {
    const fieldIds = ["procurement_team_member", "sa_assigned", "sf_assigned"];
    let allValid = true;

    fieldIds.forEach(function(fieldId) {
        const $hiddenField = $("#" + fieldId + "1"); // hidden input
        const $displayField = $("#" + fieldId);      // visible field
        ...
        if (value === "") {
            // Field empty → show error & keep mandatory class
            if (!$displayField.hasClass("V~M")) {
                $displayField.addClass("V~M");
            }
            showAssignedError(fieldId);
            allValid = false;
        } else {
            // Field has value → remove error & mandatory class
            if ($displayField.hasClass("V~M")) {
                $displayField.removeClass("V~M");
                console.log(fieldId + " V~M removed");
            }
            clearAssignedError(fieldId);
        }
    });
...
// Validate only mandatory fields (V~M)
function validateAssignedFields() {
    const fields = ["sa_assigned", "sf_assigned", "procurement_team_member"];
    let allValid = true;
    ...
    fields.forEach(function(fieldId) {
        ...
        if (!$field.hasClass("V~M")) return; // skip optional fields
        ...
        if (value === "") {
            ...
            clearAssignedError(fieldId);
            $field.removeClass("V~M"); // remove mandatory if filled
        }
    });

    toggleSaveButton(allValid);
    return allValid;
}
...
observeHiddenField("procurement_team_member");
observeHiddenField("sa_assigned");
observeHiddenField("sf_assigned");

/* Revalidate when chip is removed */
$(document).on("click", ".remove-chip", function () {
    setTimeout(validateAssignedFields, 100);
});
...
$(document).on("click", ".remove-chip", function () {
    const $btn = $(this);
    const hiddenFieldId = $btn.data("hiddenfield"); // e.g., procurement_team_member1
    const valueId = String($btn.data("id"));        // id of removed value

    const $hidden = $("#" + hiddenFieldId);

    if (!$hidden.length) return;

    // Remove the value from hidden field
    let values = ($hidden.val() || "").split(",").map(v => v.trim()).filter(v => v !== "");
    values = values.filter(v => v !== valueId);
    $hidden.val(values.join(",")); // update hidden input

    // Remove chip from DOM
    $btn.closest(".tag").remove();

    // Now validate fields immediately
    validateAssignedFields();
});

/* Observe hidden field value changes (same as reference_number1) */
...
function observeHiddenField(fieldId) {
    ...
    const observer = new MutationObserver(function () {
        console.log("Hidden field changed:", fieldId);
        setsasf(); // set mandatory fields based on team
        validateAssignedFields();
    });
...
/* Optional: initial validation on page load */
$(document).ready(function () {
    setsasf(); // set mandatory fields based on team
    validateAssignedFields();
});
