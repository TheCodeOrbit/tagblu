...
if($classarray['readonly'] == 1)
{
    ...
    $(document).ready(function (){
        ...
        flatpickr(dateInput[0], {
            ...
            defaultDate: ("{$storedDate}".trim() === "" ? null : "{$storedDate}"),  // Set default date from PHP (it should be in YYYY-MM-DD format)
...
// echo $storedDate;
  ...
  $(document).ready(function (){
      // Log the stored date to the console to ensure it's being passed correctly
      console.log("Stored Date from PHP: ", "{$storedDate}");
      console.log("fiel from PHP: ", "{$field['columnname']}");
...
// $nonce = NONCE; // Make sure it's defined early, as discussed before

// $script = "<script nonce=\"$nonce\">\n$js\n</script>";

// $this->registerJs($script, \yii\web\View::POS_END, 'js-with-nonce');
}
