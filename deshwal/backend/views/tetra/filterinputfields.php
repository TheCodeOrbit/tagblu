...
$relationName =   'editfields';
$counter = 0;
$field = $arrRender['field'];
$TableName= $arrRender['tablename'];
...
$hasadminpower = '';
	...
	if ($field["uitype"] != 2 && $field["columnname"] != 'salutation' && $field["uitype"] != 11 && $field["uitype"] != 70 && $field["uitype"] != 53 && $visible == 0) {
		...
		if ($counter == 0) {
			...
			echo '<div class="form-group  ' . $clss . ' form-field-cst section-' . $field["columnname"] . ' col-12 mb-2">';
		} else {
			echo '</div><div class="form-group  ' . $clss . ' form-field-cst section-' . $field["columnname"] . ' col-12 mb-2">';
		}
	}
	...
	if ($field["mandatory"] == 1) {
		...
		$classarray = array(
			'class' => $popupclass . ' form-control  ' . $field["typeofdata"],
			'id' => $field["fieldname"],
			'maxlength' => $field["maximumlength"],
			"value" => isset($Record->{$field["columnname"]}) ? $Record->{$field["columnname"]} : ""
		);
	...
	<?php if ($field["uitype"] != 2  && $field["columnname"] != 'salutation' && $field["uitype"] != 11 && $field["uitype"] != 70 && $field["uitype"] != 53 && $field["uitype"] != 6 && $visible == 0) { //show label if not hiden type and not radio type
		
				//echo Html::label($field["fieldlabel"] . $mandatory, $field["fieldname"], ['class' => 'control-label ', 'title' => !empty($field["description"]) ? $field["description"] : $field["fieldlabel"]]);
			}
			...
			} elseif ($field["uitype"] == 8 && $visible == 0 && $field["columnname"] != 'salutation') //simgle drop down
			{
				if ($field["columnname"] == "ownerid") {
					...
					echo $this->render('dropdown', ['baseUrl' => $baseUrl, 'classarray' => $classarray, 'field' => $field, 'fieldoptions' => $field["fieldoptions"], 'Record' => isset($Record->{$field["columnname"]}) ? $Record->{$field["columnname"]} : Yii::$app->user->id]);
				} else {
					// print_r($field["fieldoptions"]);
					// echo $read;
					if (!empty($read))
						$classarray['class'] = $classarray['class'] . ' ';
					else
						$classarray['class'] = $classarray['class'] . ' singleselect';

					echo $this->render('dropdown', ['baseUrl' => $baseUrl, 'classarray' => $classarray, 'field' => $field, 'fieldoptions' => $field["fieldoptions"], 'Record' => isset($Record->{$field["columnname"]}) ? $Record->{$field["columnname"]} : '']);
				}
							...
							'item' => function ($index, $label, $name, $checked, $value) use ($field, $read) {
								return Html::tag(
									...
									Html::checkbox($name, $checked, [
										'value' => $value,
										'class' => 'form-check-input single-edit-model-checkbox ' . $field["typeofdata"] . ' ', // Custom class for checkbox
							...
							'item' => function ($index, $label, $name, $checked, $value) use ($field, $read) {
								return Html::tag(
									...
									Html::radio($name, $checked, [
										'value' => $value,
										'class' => 'form-check-input ' . $field["typeofdata"] . ' ', // Custom class for radio button
			...
			} elseif ($field["uitype"] == 22 && $visible == 0) //multi drop down
			{
				...
				echo $this->render('multi_list', ['baseUrl' => $baseUrl, 'classarray' => $classarray, 'field' => $field, 'fieldoptions' => $field["fieldoptions"], 'Record' => isset($Record->{$field["columnname"]}) ? $Record->{$field["columnname"]} : '', 'selectedValues' => $selectedValues]);  // Include the HTML structure defined earlier
				// print_r($field["fieldoptions"]);
				...
				if ($field["fieldname"] == 'related_to_id') {
					...
					if (!empty($sourceid)) {
						if (isset($classarray['readonly']))
							...
							array_push($classarray['readonly'], '');
					}
				} else
					$val = '';
				?>
		...
		<?php } elseif ($field["uitype"] == 6 && $visible == 0) { //checkbox
				...
				if ($field["mandatory"] == 1) {
					echo Html::{$field["fieldtype"]}(
						...
						[
							'id' => $field["fieldname"],
							"class" => "form-check-input single-edit-model-checkbox " . $field['typeofdata'] . " " . $field["classname"] . " ", // Optional custom CSS class
					...
					// below line is required for single edit in summery. by using this data will set null if checkbox is not checked
					echo '<input type="hidden" name="'.$field["tablename"] . '[' . $field["fieldname"] . ']'.'" value="0">';
					echo Html::{$field["fieldtype"]}(
						$field["tablename"] . '[' . $field["fieldname"] . ']', // Name attribute
						$checked ? true : false, // Checked state
						[
			...
			if ($field["uitype"] != 2 && $field["uitype"] != 11 && $field["columnname"] != 'salutation' && $visible == 0) {
