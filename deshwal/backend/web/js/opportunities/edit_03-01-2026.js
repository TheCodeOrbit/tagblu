  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response && response.data) {
        ...
        bill_from_state_code = $("#bill_from_state_code").val();
        bill_state_code = $("#bill_state_code").val();
        gst = response.data.gst_percentage;
...
$(document).on(
  "change",
  "[id^=master_category_],[id^=sub_category_],[id^=product_name_],[id^=margin_percentage_],[id^=quantity_],[id^=cost_price_]",
  function () {
    ...
    if (changedElement.attr("id").startsWith("cost_price_") && changedElement.val()) {
...
function settotal(trid) {
  ...
  if (((cgst || sgst) || igst) && sales_price && quantity) {
