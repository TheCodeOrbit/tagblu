...
//die();
echo '<div class="row" style="padding-top:10px"><!--open first row--><div class="col-md-6"><!--open first col-->';
$relationName = 'quickcreate';
 
$counter = 0;
foreach ($Block->quickcreatefields as $field) {
	$fieldid = $field["fieldid"];
	$displaytype =  $field["quickcreate"];
	if ($hasadminpower == 1) {
		$visible =	0;
		$readonly =	0;
	} else {
	...
	if ($field["uitype"] != 2 && $displaytype != 0 && $field["uitype"] != 70 && $field["uitype"] != 53 && $visible == 0) {
		//close/open divs
		if ($counter > 0) {
			echo '</div><!-- close form group--></div><!-- close col6 div--><div class="col-md-6 inner"><div class="form-group field-uiinputs-' . $field["fieldname"] . '">';
		} else {
			// for first col 6
	...
	//check mandatory
	if ($displaytype == 2) {//visisble and mandatory
	...
	else if ($displaytype == 1) {//visisble and optional
...
?>
	...
	<?php if ($field["uitype"] != 2 && $displaytype != 0 && $field["uitype"] != 70 && $field["uitype"] != 53  && $field["uitype"] != 6 && $visible == 0) { //show label if not hiden type and not radio type
	...
	if ($field["uitype"] == 1 && $visible == 0) {
		...
		if($displaytype == 0)
			echo	 Html::hiddenInput($field["tablename"] . '[' . $field["fieldname"] . ']', '', ['id' => $field["fieldname"],]);
		else
		...
		else
		{
			if ($field["mandatory"] == 1) {
				...
				echo Html::{$field["fieldtype"]}(
					...
					$field["fieldoptions"],
					['class' => 'form-control', "prompt" => "Select " . $field["fieldlabel"], 'data-pristine-required' => 'true', 'data-pristine-required-message' => $field["fieldlabel"] . ' is required ', 'disabled' => $readonly ? true : false]
				);
			} else {
				echo Html::{$field["fieldtype"]}($field["tablename"] . '[' . $field["fieldname"] . ']', isset($Record->{$field["columnname"]}) ? $Record->{$field["columnname"]} : '', $field["fieldoptions"], ['disabled' => $readonly ? true : false, 'class' => 'form-control', "prompt" => "Select " . $field["fieldlabel"]]);
			}
		}
...
<div class="vendor-input-wrapper">
	...
	<svg class="icon-left" fill="#2F80ED" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="15" height="15" role="button" tabindex="0" id="removeTextValue" data-fieldname1="<?= $fieldname1 ?>" data-fieldname="<?= $fieldname ?>" aria-label="Remove vendor">
...
<input class="effect" style="flex-grow:1;" type="hidden" id="<?php echo $fieldname1; ?>" name="<?php echo $field["tablename"] . '[' . $field["fieldname"] . ']' ?>" value="<?php echo $ref_disp_value; ?>" readonly='readonly'>
	...
	<svg class="icon-right search-icon" width="15" height="15" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg" data-toggle="modal" data-target="#myModal22" role="button" aria-hidden="true" tabindex="0" 
	id="showCustomer1"
...
<?php 
 ...
 elseif ($field["uitype"] == 6 && $visible == 0) { //checkbox
	if($displaytype == 0)
			echo	 Html::hiddenInput($field["tablename"] . '[' . $field["fieldname"] . ']', '', ['id' => $field["fieldname"],]);
		else
		{
	...
	} elseif ($field["uitype"] == 53) //hiden created and moiied by
	{
	...
	} elseif ($field["uitype"] == 70) //hiden //created an modified datetime
	{
	...
	if ($field["uitype"] != 2 && $displaytype != 0) {
