    ...
    foreach ($block->createfields as $field) {
        ...
        if ($field["uitype"] != 2 && $field["fieldid"] != 19 && $field["uitype"] != 11 && $field["uitype"] != 70 && $field["uitype"] != 53 && $visible == 0) {
            ...
            if ($counter == 0) {
                ...
                // echo '<td class="wdinput form-group  ' . $clss . ' form-field-cst section-' . $field["columnname"] . '  ">';
                echo '<td data-col="' . $colIndex . '" class="' . $widthClass . ' form-group  ' . $clss . ' form-field-cst section-' . $field["columnname"] . '  col-pinned"> ';
            } else {
                //echo '<!--close td inner--></td><td class="wdinput form-group  ' . $clss . ' form-field-cst section-' . $field["columnname"] . ' ">';
                echo '<!--close td inner--></td><td data-col="' . $colIndex . '" class="' . $widthClass . ' form-group  ' . $clss . ' form-field-cst section-' . $field["columnname"] . ' col-pinned ">';
        ...
        <?php
        //code added by ptpatel on date 02-05-25
        if (($field['tabid'] == '69' && $field['fieldid'] == '2735') || ($field['tabid'] == '70' && $field['fieldid'] == '2765'))
            echo '<input type="hidden" id="inventory_id_' . $cnt_rows . '" name="' . $field["tablename"] . '[' . $cnt_rows . '][inventory_id]" value="" />';
        //end code added by ptpatel
        //code added by ptpatel on date 19-02-26
        if ($field['tabid'] == '67' && $colIndex == 1)
            echo '<input type="hidden" id="prod_weight_' . $cnt_rows . '" name="' . $field["tablename"] . '[' . $cnt_rows . '][prod_weight]" class = "prod_weight" value=""/>';
        //end code added by ptpatel
        ...
        <?php } elseif ($field["uitype"] == 27 && $visible == 0) { ?>
            ...
            ?>
                        ...
                        <?php echo Html::{$field["fieldtype"]}('', $ref_disp_value, $classarray); ?>
                            ...
                            if (!$classarray['readonly']): ?>
                                        <!-- Search Icon on the Right -->
                                        <svg class="icon-right related-search-icon search-icon" width="15" height="15" viewBox="0 0 24 25"
                                            fill="none" xmlns="http://www.w3.org/2000/svg" data-toggle="modal" data-target="#myModal22"
                                            role="button" aria-hidden="true" tabindex="0" data-onrefclick="<?= $onclick1 ?>" aria-label="Search vendor">
                                            <path
                                                d="M21 21.5L16.514 17.006L21 21.5ZM19 11C19 13.2543 18.1045 15.4163 16.5104 17.0104C14.9163 18.6045 12.7543 19.5 10.5 19.5C8.24566 19.5 6.08365 18.6045 4.48959 17.0104C2.89553 15.4163 2 13.2543 2 11C2 8.74566 2.89553 6.58365 4.48959 4.98959C6.08365 3.39553 8.24566 2.5 10.5 2.5C12.7543 2.5 14.9163 3.39553 16.5104 4.98959C18.1045 6.58365 19 8.74566 19 11V11Z"
                                                stroke="#2F80ED" stroke-width="2" stroke-linecap="round"></path>
                                        </svg>
