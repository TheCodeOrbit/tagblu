...
function upFileGenerateMailStatus($date_cond, $mailStatus, $modulestatus)
{
     // Assuming db_connect() returns a PDO connection
     $mycon = db_connect();

     $query = "INSERT INTO `report_files_status` (`file_created_date`, `file_type`, `status`, `created_time`, `modified_time`)
              VALUES (:date_cond, :modulestatus, :mailStatus, NOW(), NOW())";
...
function checkMailStatus($date_cond, $modulestatus)
{
     $mycon = db_connect();
     $result_count = 1;
     $query_mailstatus = "SELECT file_created_date FROM report_files_status WHERE file_type = :modulestatus AND file_created_date = :date_cond";
...
checkMailStatus($today, $callStatus);
checkMailStatus($today, $meetingStatus);
checkMailStatus($today, $quotesStatus);
checkMailStatus($today, $paymentsStatus);
checkMailStatus($today, $inspectionStatus);
checkMailStatus($today, $drillingStatus);
checkMailStatus($today, $degaussingStatus);
checkMailStatus($today, $shreddingStatus);
checkMailStatus($today, $datawipingStatus);
checkMailStatus($today, $pickupStatus);
checkMailStatus($today, $opporStatus);
checkMailStatus($today, $opporshipStatus);
checkMailStatus($today, $opporproductStatus);
checkMailStatus($today, $leadStatus);
checkMailStatus($today, $quotesditStatus);
checkMailStatus($today, $soditStatus);
checkMailStatus($today, $poditStatus);


//=============================================================
//product costing
//=============================================================
$sd_result_count = checkMailStatus($today, $sourceingdealStatus);
...
if ($sd_result_count == 0) {
     ...
     $pc_stmt->execute(['today' => $today]);

     $pc_filePath = $directory . "/sourcingdeal_productdetail_$today.csv";
     ...
     if (file_exists($pc_filePath) && filesize($pc_filePath) > 0) {
          // upFileGenerateMailStatus($today, $mailStatus, $sourceingdealStatus);
...
//==============================================================
     ...
     $sd_stmt->execute(['today' => $today]);
     $sd_filePath = $directory . "/sourcingdeal_$today.csv";
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $sourceingdealStatus);//1 - sourcing deal
...
$call_result_count = checkMailStatus($today, $callStatus);
...
if ($call_result_count == 0) {
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $callStatus);//2 - call
     // VERIFY FILE CREATED SUCCESSFULLY
...
$meet_result_count = checkMailStatus($today, $meetingStatus);
...
if ($meet_result_count == 0) {
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $meetingStatus);//3 - meeting
      // VERIFY FILE CREATED SUCCESSFULLY
...
$quotes_result_count = checkMailStatus($today, $quotesStatus);
...
if ($quotes_result_count == 0) {
     $quotesql = "select 
                         ...
                         AND DATE(quotes.createdtime) < :today
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $quotesStatus);//4 - quotes
     // VERIFY FILE CREATED SUCCESSFULLY
...
// payments
//   =====================================================================
$payments_result_count = checkMailStatus($today, $paymentsStatus);
...
if ($payments_result_count == 0) {
     $paymentql = "select 
                         ...
                         `payments`
                              ...
                              AND DATE(`payments`.createdtime) < :today 
                              order by `payments`.payments_id DESC";

     $paymentstmt = $connection->prepare($paymentql);
     $paymentstmt->execute(['today' => $today]);

     $paymentfilePath = $directory . "/payments_detail_$today.csv";
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $paymentsStatus);//5 - Payment
     // VERIFY FILE CREATED SUCCESSFULLY
...
$inspection_result_count = checkMailStatus($today, $inspectionStatus);
...
if ($inspection_result_count == 0) {
     $inspectionsql = "select 
                              ...
                              AND DATE(`inspection`.createdtime) < :today 
                                   GROUP BY `inspection`.inspection_id order by `inspection`.inspection_id DESC";

     $ins_stmt = $connection->prepare($inspectionsql);
     $ins_stmt->execute([':today' => $today]);

     $ins_filePath = $directory . "/inspection_detail_$today.csv";
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $inspectionStatus);//6 - inspection
      // VERIFY FILE CREATED SUCCESSFULLY
...
//   Drilling
// =============================================================================
$drilling_result_count = checkMailStatus($today, $drillingStatus);
...
if ($drilling_result_count == 0) {
     $drillingsql = "select 
                                   ...
                                   AND DATE(`drilling`.createdtime) < :today order by `drilling`.drilling_id DESC";


     $drilling_stmt = $connection->prepare($drillingsql);
     $drilling_stmt->execute(['today' => $today]);

     $drilling_filePath = $directory . "/drilling_detail_$today.csv";
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $drillingStatus);//7 - drilling
      // VERIFY FILE CREATED SUCCESSFULLY
...
//   Degaussing
// =============================================================================
$degaussing_result_count = checkMailStatus($today, $degaussingStatus);
...
if ($degaussing_result_count == 0) {
     $degaussingsql = "select         
                                   ...
                                   LEFT OUTER JOIN user as userpickup_spoc on (`degaussing`.pickup_spoc=userpickup_spoc.id)
                                        ...
                                        where `degaussing`.deleted=0 AND DATE(`degaussing`.createdtime) < :today order by `degaussing`.degaussinginfo_id DESC";

     $degaussing_stmt = $connection->prepare($degaussingsql);
     $degaussing_stmt->execute(['today' => $today]);

     $degaussing_filePath = $directory . "/degaussing_detail_$today.csv";
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $degaussingStatus);//8 - Degaussing
      // VERIFY FILE CREATED SUCCESSFULLY
...
//   Shredding
// =============================================================================
$shredding_result_count = checkMailStatus($today, $shreddingStatus);
...
if ($shredding_result_count == 0) {
     $Shreddingsql = "select 
                    ...
                    from 
                         ...
                         AND DATE(`shredding`.createdtime) < :today order by `shredding`.shredding_id DESC";

     $shredding_stmt = $connection->prepare($Shreddingsql);
     $shredding_stmt->execute(['today' => $today]);

     $shredding_filePath = $directory . "/shredding_detail_$today.csv";
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $shreddingStatus);//9 - Shredding
      // VERIFY FILE CREATED SUCCESSFULLY
...
//   Data Wiping
// =============================================================================
$datawiping_result_count = checkMailStatus($today, $datawipingStatus);
...
if ($datawiping_result_count == 0) {
     $datawipingsql = "select 
                         ...
                         data_wiping.activity_pincode AS 'Activity Pincode',
                              ...
                              where `data_wiping`.deleted=0 AND DATE(`data_wiping`.createdtime) < :today
                              order by `data_wiping`.datawiping_id DESC";

     $datawiping_stmt = $connection->prepare($datawipingsql);
     $datawiping_stmt->execute(['today' => $today]);

     $datawiping_filePath = $directory . "/datawiping_detail_$today.csv";
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $datawipingStatus);//10 - Data wiping
      // VERIFY FILE CREATED SUCCESSFULLY
...
// Pickup
// ========================================================================
$pickup_result_count = checkMailStatus($today, $pickupStatus);
...
if ($pickup_result_count == 0) {
     $pickupsql = "Select 
                                   ...
                                   where `pickup`.deleted=0 AND DATE(`pickup`.createdtime) < :today
                                   GROUP BY `pickup`.pickup_id order by `pickup`.pickup_id DESC";

     $pickup_stmt = $connection->prepare($pickupsql);
     $pickup_stmt->execute(['today' => $today]);

     $pickup_filePath = $directory . "/pickup_detail_$today.csv";
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $pickupStatus);//11 - Pickup
     // VERIFY FILE CREATED SUCCESSFULLY
...
// Opportunity
// ========================================================================
$oppor_result_count = checkMailStatus($today, $opporStatus);
echo "\nFile Created count for opportuntiy =$oppor_result_count";
if ($oppor_result_count == 0) {
     $pickupsql = "SELECT userownerid.username AS `Opportunity Owner`, 
          usercreatorid.username AS `Created BY`,
          ...
          where `opportunity`.deleted=0 AND  `opportunity`.is_temp =0  AND DATE(`opportunity`.createdtime) < :today group by `opportunity`.opportunity_id order by `opportunity`.opportunity_id DESC";

     $pickup_stmt = $connection->prepare($pickupsql);
     $pickup_stmt->execute(['today' => $today]);

     $pickup_filePath = $directory . "/opportunity_$today.csv";
     $pickup_fp = fopen($pickup_filePath, 'w');
     if (!$pickup_fp) {
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $opporStatus);
     // VERIFY FILE CREATED SUCCESSFULLY
     if (file_exists($pickup_filePath) && filesize($pickup_filePath) > 0) {
          upFileGenerateMailStatus($today, $mailStatus, $opporStatus);
...
// ========================================================================
$oppor_ship_result_count = checkMailStatus($today, $opporshipStatus);
...
if ($oppor_ship_result_count == 0) {
     ...
     where     `opportunity`.deleted=0 AND  `opportunity`.is_temp =0  AND DATE(`opportunity`.createdtime) < :today  order by `opportunity`.opportunity_id DESC";

     $pickup_stmt = $connection->prepare($pickupsql);
     $pickup_stmt->execute(['today' => $today]);

     $pickup_filePath = $directory . "/opportunity_shiptoaddress_detail_$today.csv";
     $pickup_fp = fopen($pickup_filePath, 'w');
     if (!$pickup_fp) {
     ...
     upFileGenerateMailStatus($today, $mailStatus, $opporshipStatus);
}

//  =======================================================================
// Opportunity Product Detail
// ========================================================================
$oppor_product_result_count = checkMailStatus($today, $opporproductStatus);
...
FROM opportunity_product_detail
  ...
  left join oppr_days as add_product_delivery_timeline on opportunity_product_detail.add_product_delivery_timeline=add_product_delivery_timeline.days_id
     ...
     $pickup_filePath = $directory . "/opportunity_product_detail_$today.csv";
     $pickup_fp = fopen($pickup_filePath, 'w');
     if (!$pickup_fp) {
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $opporproductStatus);//11 - Pickup
     // VERIFY FILE CREATED SUCCESSFULLY
     if (file_exists($pickup_filePath) && filesize($pickup_filePath) > 0) {
          upFileGenerateMailStatus($today, $mailStatus, $opporproductStatus);
...
//LEad Detail
// ========================================================================
$lead_result_count = checkMailStatus($today, $leadStatus);
...
if ($lead_result_count == 0) {
  ...
  where leadinformation.deleted = 0 AND DATE(`leadinformation`.createdtime) < :today  
  order by leadinformation.leadid desc";

     $pickup_stmt = $connection->prepare($pickupsql);
     $pickup_stmt->execute(['today' => $today]);

     $pickup_filePath = $directory . "/lead_detail_$today.csv";
     $pickup_fp = fopen($pickup_filePath, 'w');
     if (!$pickup_fp) {
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $leadStatus);//11 - Pickup
     // VERIFY FILE CREATED SUCCESSFULLY
     if (file_exists($pickup_filePath) && filesize($pickup_filePath) > 0) {
          upFileGenerateMailStatus($today, $mailStatus, $opporproductStatus);
...
//Quotes DevIT Detail
// ========================================================================
$quotesdit_result_count = checkMailStatus($today, $quotesditStatus);
...
if ($quotesdit_result_count == 0) {
 ...
 AND DATE(`quotes_dit`.createdtime) < :today  
  order by quotes_dit.quotes_dit_id desc";

     $pickup_stmt = $connection->prepare($pickupsql);
     $pickup_stmt->execute(['today' => $today]);

     $pickup_filePath = $directory . "/quotesdit_detail_$today.csv";
     $pickup_fp = fopen($pickup_filePath, 'w');
     if (!$pickup_fp) {
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $quotesditStatus);
     // VERIFY FILE CREATED SUCCESSFULLY
     if (file_exists($pickup_filePath) && filesize($pickup_filePath) > 0) {
          upFileGenerateMailStatus($today, $mailStatus, $quotesditStatus);
...
// ========================================================================
$sodit_result_count = checkMailStatus($today, $soditStatus);
...
if ($sodit_result_count == 0) {
 ...
 AND DATE(`salesorder_dit`.createdtime) < :today  
  order by salesorder_dit.salesorder_dit_id desc";

     $pickup_stmt = $connection->prepare($pickupsql);
     $pickup_stmt->execute(['today' => $today]);

     $pickup_filePath = $directory . "/saleordersdit_detail_$today.csv";
     $pickup_fp = fopen($pickup_filePath, 'w');
     if (!$pickup_fp) {
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $soditStatus);//11 - Pickup
     // VERIFY FILE CREATED SUCCESSFULLY
     if (file_exists($pickup_filePath) && filesize($pickup_filePath) > 0) {
          upFileGenerateMailStatus($today, $mailStatus, $quotesditStatus);
...
// ========================================================================
$podit_result_count = checkMailStatus($today, $poditStatus);
...
if ($podit_result_count == 0) {
 ...
 AND DATE(`purchase_order_dit`.createdtime) < :today  
  ...
  order by purchase_order_dit.purchaseorder_dit_id desc;";
     ...
     $pickup_filePath = $directory . "/purchaseordersdit_detail_$today.csv";
     $pickup_fp = fopen($pickup_filePath, 'w');
     if (!$pickup_fp) {
     ...
     // upFileGenerateMailStatus($today, $mailStatus, $poditStatus);
     // VERIFY FILE CREATED SUCCESSFULLY
     if (file_exists($pickup_filePath) && filesize($pickup_filePath) > 0) {
          upFileGenerateMailStatus($today, $mailStatus, $poditStatus);
