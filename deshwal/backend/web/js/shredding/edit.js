$(document).ready(function () {
  ...
  $(document).on("change", "select[id^='shredding_completed_']", function () {
    
    let index = $(this).attr("id").match(/shredding_completed_(\d+)/)[1];
  ...
  $(document).on("click", ".shredding-completed ", function () {
        let data = {
            Recordid: $("#Recordid").val(),
            _csrf: $("#csrfToken").val(),
            shredding_completed: "Yes",
        };
        //alert("dgd");
        $("#loading-overlay").css('display', 'grid');
        $.ajax({
            type: "POST",
            url: "shreddingcompleted",
            data: data,
            success: function (data) {
                if (data.status === "success") {
                  $(".shredding_completed ,.add-lead-btn2").remove()
  ...
  $(document).on("click", ".shredding-client-sign", function () {
        let data = {
            Recordid: $("#Recordid").val(),
            _csrf: $("#csrfToken").val(),
            shredding_asset_details: "Yes",
        };
    $("#loading-overlay").css('display', 'grid');
    $("#detailViewGeneralLabel").text("shredding Assets")
        $.ajax({
            type: "POST",
            url: "shreddingassets",
            data: data,
  ...
  $(document).on("click", "#detail-view-general-submit", function (e) {
    ...
    if (signatureData) {
      let data = {
        ...
        $("#loading-overlay").css('display', 'grid');
        $.ajax({
            url: 'shreddingclientsign',
            type: 'POST',
            data: data,
            success: function (data) {
                if (data.status === "success") {
                  $(".shredding-client-sign,.add-lead-btn2").remove()
                  $("#detail-view-general-info").modal("hide")
                  showSuccessMessage(data.message || "Updated successfully")
                  location.reload();
                } else {
