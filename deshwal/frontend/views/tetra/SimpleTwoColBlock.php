...
<?php if($Block->blockid !=6){ ?>
					<div class="body-outline h-auto mb-3<?php 
	/*if ($Block->blockid==98) { 
		echo "finance-grid-table-1";
	}elseif ($Block->blockid==102) {
		echo "finance-grid-table-2";
	}elseif($Block->blockid==103){
		echo "finance-grid-table-3";
	}elseif($Block->blockid==104){
		echo "finance-grid-table-3";
	}elseif($Block->blockid==105){
		echo "finance-grid-table-5";
	}elseif($Block->blockid==106){
		echo "finance-grid-table-6";
	}else{
		echo "finance-grid-table-7";
	}*/?>" style="height:auto;">
						<?php } ?>
								...
								<?php $counter=0;foreach($Block->Fields as $field) {
									if($counter%2 == 0){ ?>
										...
										<?php if($field['uitype']!=2){?>
											<div class="input-heading simple-table__container__heading" style="width:50%">
												<?php echo $form->labelEx($model,$field['fieldname'], array ()); ?>
											</div>
										<?php } ?>
										<?php 
											if($field['uitype']==1){
												echo $form->{$field[fieldtype]}($model,$field['columnname'], array ('autocomplete' => 'new-password','class' =>'input-border  w-50 simple-table__container__input','value'=>$Record->{$field['columnname']}?$Record->{$field['columnname']}:''));
												...
												echo $form->{$field['fieldtype']}($model,$field['columnname'],array (
													'autocomplete' => 'new-password','class' => 'effect  w-50 '.$field["classname"],'value'=>$Record->{$field['columnname']}));
