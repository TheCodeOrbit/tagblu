...
<div>	
	...
	<div class="topcontent-details"><!-- topcontent starts -->
		<div class="body-menu d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex justify-content-between align-items-center">
				...
				<?php echo $form->error($model,$field['fieldname'], array('class'=>'ajxwarning errorMessage')); ?>
							<?php echo $form->{$field['fieldtype']}($model,$field['fieldname'], $field['fieldoptions'],array('class' => 'form-select','value'=>$field['columnname'],'empty'=>'Select '.$field['fieldlable'],'options' => array($Record->{$field['columnname']}=>array('selected'=>true)))); ?>

				<?php elseif($field['uitype']==1):?>

				<?php echo $form->error($model,$field['fieldname'], array('class'=>'ajxwarning errorMessage')); ?>
				<?php 
					if(($ModuleName=="obr_contractor" and $field['fieldname']=="total_nooftrips") or ($ModuleName=="cecontractor" and $field['fieldname']=="total_nooftrips")):
				...
				<?php 
					echo $form->error($model,$field['fieldname'], array('class'=>'ajxwarning errorMessage')); echo $form->hiddenField($model,$field['columnname'], array ('class' => $field['classname'],'value'=>$Record->{$field['columnname']})); echo "<input type='text' name={$field[columnname]} id={$field[columnname]} class='$field[classname]' value=''/> ";
				?>
				<?php endif;?>
				<?php if($counter%2==0):?>
			</div>
		...
		<?php if($ActionName=="Create"){ ?>
			<button type="button" id="showModel" class="btn btn-primary btn-custom" data-bs-toggle="modal" data-bs-target="#exampleModal">SAVE</button>
		<?php }  else {?>
			<?php echo CHtml::submitButton('Discard', array('name' => 'btncancel','class' => 'btn btn-outline-danger btn-custom me-5 close')); ?>
			<?php echo CHtml::submitButton('Save', array('class' => 'btn btn-primary btn-custom close')); ?>	
		<?php } ?>
	</div><!-- topcontent ends -->
		...
		<div class="body-outline">
			<table id="Tbl_<?php echo $obj_name;?>">
                ...
                <?php
					...
					if($cnt_multiple_product>0){
						foreach($Record[Multiple_Records][$Block->blockid] as $MPkey=> $Multiple_Record){ $tax_html="";?>
									...
									<?php }elseif($Field['uitype']==8){
										...
										<?php } else { ?>
											<td class="<?php echo $Field->td_classname;?>">
												...
												<?php	//echo $form->textField(${$obj_name},"[$MPkey]$Field->fieldname", array ('class' => $Field->classname,'value'=>$Multiple_Record->{$Field->fieldname})); ?>
												<?php echo $form->dropDownList(${$obj_name},"[$MPkey]$Field->fieldname", $Field['fieldoptions'],array('class' => 'form-select','aria-label'=>'select example','value'=>$Multiple_Record->{$Field->fieldname},'empty'=>'Select an Option','options' => array($Multiple_Record->{$Field->fieldname}=>array('selected'=>true)))); ?>
											</td>
							...
							<?php }//Field foreach close ?>
								...
								<td class="<?php echo $obj_name;?>Delete text-center" id="<?php echo $Block->Fields[0]->tablename.'_'.$MPkey.'_Delete'?>"><svg viewBox="0 0 18 19" class="action-icon">
                    ...
                    <div class="add-btn d-flex jc-cntr align-items-center" id="Add<?php echo $obj_name;?>" rel="<?php echo $Block->blockid;?>">
