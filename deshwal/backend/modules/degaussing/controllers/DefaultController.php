...
namespace backend\modules\degaussing\controllers;

use common\controllers\ModuleController;
use backend\models\AccessCheck;
use app\models\EditModel;
use app\models\Degaussing;
use app\models\DegaussingAssetDetails;
...
class DefaultController extends ModuleController
{
    ...
    public $ModuleName='degaussing';
    public $FieldId='degaussinginfo_id';
    public $TableName='degaussing';
    public $TabLabel='Degaussing';
    ...
    {   
        ...
        if (!empty($columns)) {
            ...
            if(!empty($serviceDetailData)){
                ...
                $command = $connection
                ->createCommand("SELECT * FROM servicedetail_details WHERE service_type = 1 and servicedetail_id = :servicedetail_id and deleted=0")
    ...
    public function actionGetlocationddress()
    {
        ...
        if(!empty($sourcing_deal)){
            ...
            if(!empty($serviceDetailData)){
                ...
                $columns["servicedetail_id"] = $servicedetail_id;
                $command = $connection
                ->createCommand("SELECT * FROM servicedetail_details WHERE service_type = 1 and servicedetail_id = :servicedetail_id and service_to_location=:service_to_location and deleted=0")
                ->bindValue(":servicedetail_id", $servicedetail_id)
    ...
    public function actionDegaussingcompleted(){
        try{
            $transaction_enabled = false;
            if (!(isset($_POST['degaussing_completed']) && $_POST["degaussing_completed"] == "Yes")) {
            ...
            $oldAttributes = Yii::$app->db->createCommand("select * from `degaussing` where degaussinginfo_id=:degaussinginfo_id")
                ->bindValue(":degaussinginfo_id", $Record)
                ->queryOne();
            if (empty($oldAttributes)) {
                throw new Exception("No record found");
            }
            $current_degaussing_status = $oldAttributes["degaussing_status"];
            ...
            if($current_degaussing_status != 4){
                throw new Exception("Invalid action, current status is not 'Degaussing in process/ Activity in process'");
            }
            if($fe_user != $id){
            ...
            $assets_query = DegaussingAssetDetails::find()->where(['degaussinginfo_id' => $Record, 'deleted' => 0]);
            ...
            if ($assets_count > 0) {
                ...
                foreach ($assets_data as $ad) {
                    ...
                    if (empty($ad->image_before_activity)) {
                        throw new Exception("'Before Activity Image' is required for all records in the 'ASSET DETAILS' section");
                    }
                    if (empty($ad->image_after_activity)) {
                        throw new Exception("'After Activity Image' is required for all records in the 'ASSET DETAILS' section");
                    }
                }
            } else {
            ...
            $new_degaussing_status = 5;
            $newattributes = array("degaussing_status" => $new_degaussing_status, "completed_date" => date("Y-m-d"));

            $sql = "UPDATE degaussing set degaussing_status = :degaussing_status,completed_date=:completed_date,modifiedtime = :modifiedtime,modifiedby = :modifiedby where degaussinginfo_id = :id";
            $status = Yii::$app->db->createCommand($sql)
                ->bindValue(":degaussing_status", $new_degaussing_status)
    ...
    public function actionDegaussingassets(){
        try{
            $transaction_enabled = false;
            if (!(isset($_POST['degaussing_asset_details']) && $_POST["degaussing_asset_details"] == "Yes")) {
                throw new Exception("Invalid Action");
            }
            $role = Yii::$app->user->identity->role ?? null;
		    $id = Yii::$app->user->id;
            ...
            $transaction = Yii::$app->db->beginTransaction();
            $transaction_enabled = true;
            $oldAttributes = Yii::$app->db->createCommand("select * from `degaussing` where degaussinginfo_id=:id")
                ->bindValue(":id", $Record)
                ->queryOne();
            if (empty($oldAttributes)) {
                throw new Exception("No record found");
    ...
    public function actionDegaussingclientsign(){
        try{
            $transaction_enabled = false;
            ...
            $hdd_completed = (int)$oldAttributes["hdd_completed"];
            $client_sign = $oldAttributes["image"];
            $degaussing_no = $oldAttributes["degaussing_no"];
            $signature_file_name = "$degaussing_no^client_signature.png";
            if(empty($hdd_count)){
                throw new Exception("Invalid action ! HDD Count is not found");
            }
            if($current_degaussing_status != 4){
            ...
            $result = EditModel::saveGeneratedFiles($image_data[1], $signature_file_name,$Record,'degaussing',$oldAttributes, $new_attributes,"image/png");
            if (!$result['success']) {
            ...
            $sql = "UPDATE degaussing set image = :image,modifiedtime = :modifiedtime,modifiedby = :modifiedby where degaussinginfo_id = :id";
            $status = Yii::$app->db->createCommand($sql)
        ...
        public function actionGetsourcingdetail(){
            ...
            $command = $connection->createCommand("
                             ...
                             WHERE sourcingdeal_id = :sourcingdeal_id and service_type=1
