		...
		<?php //echo CHtml::submitButton('Save', array('class' => 'btn btn-primary input-save','id'=>'showModel','data-bs-toggle'=>'modal','data-bs-target'=>'#exampleModal')); ?>	
                    ...
                    <button type="button" id="showModel" class="btn btn-primary input-save" data-bs-toggle="modal" data-bs-target="#exampleModal">SAVE</button>
									<?php }  else {?>
										...
										<?php echo CHtml::submitButton('Discard', array('name' => 'btncancel','class' => 'btn btn-primary input-save me-5 close')); ?>
									<?php } ?>
                </div>
            </div>
				<!--<div class="row">
                        ...
                        <?php
								...
								<?php } elseif($Field['uitype']==8){
									...
									<td class="<?php echo $Field->td_classname;?>">
										...
										<?php echo $form->dropDownList(${$obj_name},"[$MPkey]$Field->fieldname", $Field['fieldoptions'],array('class' => 'form-select w-150px','aria-label'=>'select example','value'=>$Multiple_Record->{$Field->fieldname},'empty'=>'Select an Option','options' => array($Multiple_Record->{$Field->fieldname}=>array('selected'=>true)))); ?>
									</td>
...
<script>
	$(document).ready(function(){
		...
		if(mode=='Create'){
			...
			var url=geturl+'index.php/dailydrilling/ProductBlock';
								      	$.ajax({
												type: 'POST',
												url: url,
												data:'cnt_multiple_product='+cnt_multiple_product+'&blockid='+block_id,
												success:function(data){ 
												$('#Tbl_drilling_data tr:last').after(data);
												$(".drilling_type option[value='1']").attr("selected","selected");
												$('.drilling_type').trigger("chosen:updated");
												$(document.body).on('change', '.drilling_type' ,function(){
															code	= $(this).attr('id');
															codevals= $(this).val();
															seq_no	= code.substring(14,code.indexOf('_drilling_type'));
															if(codevals == 2 || codevals =='' ){
															$("#drilling_data_"+seq_no+"_bench_height").attr('readonly',true);
															}
															else{
															$("#drilling_data_"+seq_no+"_bench_height").attr('readonly',false);
															}
												});
											  cnt_multiple_product=cnt_multiple_product+1;
												$.getScript(geturl + '/js/Tetra/chosenCode.js');
