...
function pickupStatusClass($pickup_status){
    if(empty($pickup_status)) return "status";
    if($pickup_status == 8) return "status done";
    if($pickup_status == 9) return "status text-danger bg-danger-subtle";
...
<nav aria-label="breadcrumb" class="mt-2">
    <ol class="breadcrumb rounded-3">
        ...
        <li class="breadcrumb-item">
            <a class="link-body-emphasis text-decoration-none gray-shade-2-breadcrum">Pickup</a>
        </li>
    </ol>
</nav>
 <h4>Overview</h4>
    <div class="overview">
        <div class="overview-card active-card">
            <p class="dasbhoard-header">Total Pickup</p>
            <div class="d-flex align-items-baseline">
                <p class="dashboard-figure"><?= $total_pickups??0;?></p>
        ...
        <div class="overview-card completed-card">
            <p>Pickup Delivered</p>
            <div class="d-flex align-items-baseline">
                <p class="dashboard-figure"><?= $pickup_delivered??0;?></p>
                <div class="custom-green-text">
                    <span class="ms-2">+ 0.00%</span> <i class="fa-solid fa-arrow-up"></i>
                </div>
            </div>
...
<h4>Pickups</h4>
    <div class="table-container">
        <table class="table table-hover custom-table-border">
            ...
            <tbody>
                <?php foreach($pickupData as $pdata){ 
                    $pickup_class = pickupStatusClass($pdata["pickup_status"]);
                ?>
                <tr>
                    <td class="checkbox"><input type="checkbox"></td>
                    <td><?php echo $pdata["pickup_no"]??"";?></td>
                    <td><?php echo $pdata["address"]??"";?></td>
                    <td><?php echo $pdata["spoc_email"]??"";?></td>
                    <td><span class="<?php echo $pickup_class??"";?>"><?php echo $pdata["pickup_status_text"];?></span></td>
                </tr>
                <?php } ?>
            </tbody>
        </table>
