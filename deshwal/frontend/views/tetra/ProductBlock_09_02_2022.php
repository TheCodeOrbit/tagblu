...
foreach($Block->Fields as $key=> $Field):
	...
	<?php 	if(strpos($Field->classname, 'ReadOnly') !== false){
			...
			echo CHtml::textField($attr_name, '',array('id'=>$attr_id, 'class'=>$Field->classname.' input-border form-control b','readonly'=>true));
				...
				//print_r($Field);
				// echo $Field->fieldid;
				if($Field->fieldid==1899 || $Field->fieldid==280 || $Field->fieldid==1806 || $Field->fieldid==2075){
			...
			echo CHtml::textField($attr_name, '',array('id'=>$attr_id, 'class'=>$timeformat.' input-border form-control '.$attr_class, 'placeholder'=>$p_holder));
			}
		}
	...
	<?php elseif($Field['uitype']==14):?>
		...
		<td>
			...
			<button type="button" value="Stoppage reason"  class="btn btn-primary form-control target-button" 
			data-bs-toggle="modal" data-bs-target="#<?php echo $obj_name.'_'.$cnt_multiple_product.'_'.$Field->fieldname.'_reason'; ?>">Stoppage</button>
			<div class="error-container hide">All errors displayed Here</div>
						...
						<div class="modal-body" data-que="<?php echo $cnt_multiple_product;?>">
								...
								<div class="position-relative">
									<div class="d-flex justify-content-between anchor-class">
										<select class=" input-border form-control" name="<?php echo $Field->tablename;?>[<?php echo $cnt_multiple_product;?>][reason][]">
										...
										<input placeholder="hh:mm" maxlength="8" pattern="^((\d+:)?\d+:)?\d*$" class="<?php echo $Field->tablename;?> input-border form-control timeformat" name="<?php echo $Field->tablename;?>[<?php echo $cnt_multiple_product;?>][loss_hour][]" <?php echo $Field->fieldid;?>>
									</div>
								</div>
						</div>
...
//$fieldoptions=$PickList->getPickListOption($ModuleName);
			...
			<div class="d-flex flex-col">
				...
				<input type="text" value="<?php echo $refValue[$key];?>" id="<?php echo $Field->fieldname.$MPkey;?>" name ="<?php echo $Field->columnname.$MPkey;?>" size=12 class="<?php echo $Field->classname;?>" readonly="readonly" autocomplete="off" style="width: 150px;">
