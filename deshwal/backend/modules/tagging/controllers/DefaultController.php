            ...
            */
            $rows = (new \yii\db\Query())
                ->select([
                    "TO_BASE64(inventory.inventory_id) as `ID`",
                    'products.product_name as `Product Name`',
                    'prod_category.prod_category_value as `Category`',
                    'prod_sub_catagory.sub_catagory_value as `Sub Category`',
                    'inventory.serial_number as `Serial Number`',
                    'inventory.tag_number as `Tag Number`',
...
public function actionUploadinventorycsv($Recordid = null, $Productid = null, $Subcategory = null)
{
    ...
    $requiredHeaders = [
        "ID", "Product Name","Category", "Sub Category",
    ...
    foreach ($csvData as $r) {
        $csvSerials[] = strtolower($r[4] ?? "");   // Serial Number
        $csvTags[]    = strtolower($r[5] ?? "");   // Tag Number
    }
    ...
    $validStatus = ["sticker removal pending","cleaning require","iqc require"];
    ...
    foreach ($csvData as $index => $row) {
        ...
        // ---- TAG & STATUS RULES As PER NEW CHANGES v11 - 240 ----

        // 1) If both Tag and Status are empty → skip row completely
        if ($tagLower === "" && trim($status) === "") {
             $emptyRows++;
            continue;
        }

        // 2) If Status exists but Tag missing → error
        if (trim($status) !== "" && $tagLower === "") {
            $errors[$index][] = "Tag Number is required at Row $rowNum.";
        }

        // 3) If Tag exists but Status missing → error
        if ($tagLower !== "" && trim($status) === "") {
            $errors[$index][] = "Status is required at Row $rowNum.";
        }
        // ---- As PER NEW CHANGES v11 - 240 ----
        // ---- Required fields ----
        ...
        //as per v11-313 code change by ptpatel on date 01-04-2026
        $validSubcategories = ['mobile','mouse','headphones','laptopbag','keyboard','mobilecharger','laptopcharger','laptopbattery','datacard'];
        $validCategories = ['cable&wires']; // add if needed

        // normalize values
        $subCatNormalized = strtolower(str_replace(' ', '', $subCat));
        $categoryNormalized = strtolower(str_replace(' ', '', $category));

        if (
            $serialLower === "" &&
            !in_array($subCatNormalized, $validSubcategories) &&
            !in_array($categoryNormalized, $validCategories)
        ) {
            $errors[$index][] = "Serial Number is required at Row $rowNum.";
        }
        //end code added for v11-313 by ptpatel on date 01-04-2026
        ...
        if ($status !== '' && !in_array(strtolower(trim($status)), $validStatus, true)) {
            $errors[$index][] = "Status '$status' is not allowed at Row $rowNum.";
        }
        ...
        //this if condition added for v11 -313 on date 01-04-2026
         if (
            isset($seenSerial[$serialLower]) &&
            !in_array($subCatNormalized, $validSubcategories) &&
            !in_array($categoryNormalized, $validCategories)
        ) {
            //this if condition added for v11 -313 on date 01-04-2026
        ...
        if (in_array($serialLower, $dbSerials) && $serialLower !='') {
            ...
            foreach ($rows as $row) {
                ...
                if ($counter === 0) {
                    ...
                    if (!$checkgrn) {
                        return [
                            "status" => "error",
                            "message" => "GRN No., Product and Subcategory mismatch with uploaded CSV",
                            "redirect" => Yii::$app->urlManager->createUrl(['tagging/dashboard'])
                        ];
                    }
                }
