...
<div id="BatchList" class="modal fade" role="dialog"></div>
 ...
 <?php 
			...
			<div class="body-container">
                <div data-simplebar class="body-outline height-full-body">
                    ...
                    <table id="Tbl_<?php echo $obj_name;?>">
						...
						if(($ModuleName=='water_management' || $ModuleName=='screening_crushing') && $ActionName=="Create"){
		...
		//$cnt_multiple_product=1;
				...
				foreach($Record[Multiple_Records][$Block->blockid] as $MPkey=> $Multiple_Record){ $tax_html="";?>
                        <?php
							...
							// print_r($Multiple_Record->reasons);exit;
						?>
                        <tr class="prodlist">
                        <?php
...
<input  type="hidden" name="<?php echo $obj_name.'['.$MPkey.']'.'['.$Field->fieldname.'create]'; ?>" id="<?php echo $obj_name.'_'.$MPkey.'_'.$Field->fieldname.'_jqdtEditoption'; ?>" value="2" />
                    ...
                    <div class="add-btn btn-primary d-flex jc-cntr align-items-center" id="Add<?php echo $obj_name;?>" rel="<?php echo $Block->blockid;?>">
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
		...
		<?php echo CHtml::submitButton('Submit', array('class' => 'btn btn-primary input-save me-5 close')); ?>	
			...
			<div class="modal fade" id="<?php echo $obj_name.'_'.$reason_counter.'_'.$Field->fieldname.'_reason';?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
											<div class="modal-dialog modal-dialog-centered modal-xl">
												<div class="modal-content">
													<div class="body-outline">
													<div class="modal-header jc-cntr">
													</div>
													<div class="modal-body" data-que="<?php echo $reason_counter;?>">
														<div class="row">
															<div class="col-8 bg-info text-center">Stopage Reason</div>
															<div class="col-2 bg-info text-center">Hours</div>
														</div>
														<?php 
														foreach($Multiple_Record->reasons as $reason_record){ ?>
														<div class="row anchor-class mt-2">
															<div class="col-8">
																<select class="input-border form-control" name="screening_crushing_data[<?php echo $reason_counter;?>][reason][]">
																	<?php $PickList=new PickList;//$Field->tablename
																		//$parent_record = Yii::app()->request->getParam('Record');
																		$fieldoptionsReason=$PickList->getPickListReasonOptionEdit($Field->tablename,$reason_record->sc_reason_id,$reason_record->reason_id);
																		print_r($fieldoptionsReason);
																	?>
																</select>
															</div>
															<div class="col-2">
																<input value="<?php echo $reason_record->loss_hour; ?>" class="input-border form-control" name="screening_crushing_data[<?php echo $reason_counter;?>][loss_hour][]">
															</div>
														</div>
													<?php } ?>
													</div>
													<div class="modal-footer jc-cntr">
														<div class="add-btn btn-primary d-flex jc-cntr align-items-center add-rows-to-pop" >
															<svg width="16" height="16" viewBox="0 0 16 16">
																<path d="M6 16H10V10H16V6H10V0H6V6H0V10H6V16Z" fill="#ffffff"/>
															</svg>                        
														</div>
