...
.form-container-cst {
 display: flex;
  ...
  justify-content: flex-start; /* Align fields at the start */
}

.form-field-cst {
  flex: 1 1 calc(50% - 16px); /* Default to 50% width */
  max-width: calc(50% - 16px); /* Prevent from going beyond 50% */
...
<input class="effect" style="flex-grow:1;" type="hidden" id="<?php echo $fieldname1; ?>" name="<?php echo $field["tablename"] . '[' . $field["fieldname"] . ']' ?>" value="<?php echo $ref_hid_value; ?>" readonly='readonly'>
			<?php echo   Html::{$field["fieldtype"]}('', $ref_disp_value, $classarray);?>


	<!-- Search Icon on the Right -->
	<svg class="icon-right search-icon simple" width="15" height="15" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg" data-toggle="modal" data-target="#myModal22" role="button" aria-hidden="true" tabindex="0" onclick="showCustomer1('<?= $fieldname1 ?>','<?= $fieldname ?>','<?= $getRelatedDisplayFieldName; ?>','<?= $relatedmodulename; ?>',<?= $Block->blockid;;?>)" aria-label="Search vendor">
