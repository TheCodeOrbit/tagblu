...
var observer = new MutationObserver(function (mutationsList) {
  for (var mutation of mutationsList) {
    if (mutation.type === "attributes" && mutation.attributeName === "value") {
      getvendoraccount();  
      getproductdetail(); //this is added while creating document by ptpatel on date 31-12-2025 after discussion with deepika ma'am,    
...
async function getproductdetail() {
  ...
  try {
    ...
    if (response && response.data) {
      ...
      $('#productTable' + 105 + ' tbody').empty();//this is added while creating document by ptpatel on date 31-12-2025 after discussion with deepika ma'am,    
      ...
      for (let i = 0; i < response.data.length; i++) {
        ...
        if (lastRow.length > 0 && currentRow !== rowIndex) {
          ...
          if (productNameInput.length > 0) {
            ...
            //code added by ptpatel on date 19-11-2025 to resolve issue of quoteitem
...
function setTotalAmount() {
  ...
  $(".product_name").each(function () {
    // Get the suffix from the id (assuming it's in the form of product_name_1, product_name_2, etc.)
    var suffix = $(this).attr("id").match(/\d+$/) ? $(this).attr("id").match(/\d+$/)[0] : "";

    // Perform your calculations for the visible input
    ...
    // Update the totals based on these values
    ...
    console.log('total_igst_amount=' + total_igst_amount + ', add ' + igst_amount);

    // Update the form values with the new totals
    $("#basic_cp").val(basic_cp.toFixed(2));
    $("#total_cgst_amount").val(total_cgst_amount.toFixed(2));
    $("#total_sgst_amount").val(total_sgst_amount.toFixed(2));
    $("#total_igst_amount").val(total_igst_amount.toFixed(2));
    $("#basic_cp").val(basic_cp.toFixed(2)); // This line seems to be duplicated; consider removing one of them
});
...
$(document).ready(function () {
  ...
  //get Bill to Address info code added by ptpatel on date 04-08-25
  // Create a MutationObserver to detect changes to the input vendor account
  var targetNode = document.getElementById("qu_bill_location_name1");
  var observer = new MutationObserver(function (mutationsList) {
    for (var mutation of mutationsList) {
      if (mutation.type === "attributes" && mutation.attributeName === "value") {
        getBilltoAddressinfo();
        console.log("business_entity value changed to:", targetNode.value);
      }
    }
  });
  ...
  var config = { attributes: true };
  observer.observe(targetNode, config);
  function getBilltoAddressinfo() {
    $("#qu_bill_address").val("");
    $("#qu_bill_state").val("");
    $("#qu_bill_pin_code").val("");
    $("#qu_bill_warehouse_name").val("");
    $("#qu_bill_city").val("");
    $("#qu_bill_state_code").val("");
    $("#qu_bill_gstin_no").val("");

    data = {
      qu_bill_location_name: $("#qu_bill_location_name1").val(),
      _csrf: $("#csrfToken").val(),
    };

    $.ajax({
      type: "POST",
      url: "getbilltoaddressinfo",
      // async:false,
      data: data,
      success: function (response) {
        console.log(response); // Log the entire response to check its structure
        ...
        // Check if the data object exists and contains 'first_name'
        if (response && response.data) {
          $("#qu_bill_warehouse_name").val(response.data.warehouse_name);
          $("#qu_bill_address").val(response.data.address);
          $("#qu_bill_state").val(response.data.state);
          $("#qu_bill_state_code").val(response.data.statecode);
          $("#qu_bill_gstin_no").val(response.data.gstn);
          $("#qu_bill_pin_code").val(response.data.pincode);
          $("#qu_bill_city").val(response.data.city);
        } else {
          console.log("Invalid response format or missing data");
        }
      },
  ...
  function getTCSPercentageAndTCSAmount() {
    ...
    let sourcing_deal_id= $("#related_to_id1").val();
    data = {
  ...
  if (val === "1") {
    ...
    $stage.off('select2:selecting').on('select2:selecting', function(e) {
      ...
      if (selectingVal !== "1" && selectingVal !== "4") {
        e.preventDefault();
      }
    });
