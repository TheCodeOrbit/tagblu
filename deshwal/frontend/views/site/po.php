...
function statusClass($pickup_status)
{
    if (empty($pickup_status))
        return "status";
    if ($pickup_status == "Approved" || $pickup_status == 3)
        return "status done";
    if ($pickup_status == 4)
...
<nav aria-label="breadcrumb" class="mt-2">
    <ol class="breadcrumb rounded-3">
        ...
        <li class="breadcrumb-item">
            <a class="link-body-emphasis text-decoration-none gray-shade-2-breadcrum">Purchase Order</a>
        </li>
    </ol>
</nav>
<h4>Overview</h4>
<div class="overview">
    <div class="overview-card active-card">
        <p class="dasbhoard-header">Total Purchase Order</p>
        <div class="d-flex align-items-baseline">
            <p class="dashboard-figure">
                <?php
                if ($static_logic) {
                    echo "10";
                } else if (isset($total_count)) {
                    echo $total_count;
                } else {
                    echo "0";
                }
...
<div class="table-container">
    <table class="table table-hover custom-table-border">
        ...
        <tbody>
            <?php foreach ($purchaseOrderData as $pdata) {
                ...
                <tr>
                    <?php if ($static_logic) {
                        ...
                        $status_class = statusClass($pdata["stage"]);
                        if ($pdata['status'] == 3) {
                            $pdf_name = $pdata["po_no"] . "_PO_deshwal.pdf";
                            // Generate URL to the controller action
                            $pdf_url = Url::to(['site/downloadpo', 'file' => $pdf_name], true);

                            // Create HTML link
                            $pdf_link = "<a href='$pdf_url' target='_blank'>Download PDF</a>";
                        }
                        else $pdf_link = '';
                        ...
                        <td><?php echo $pdata["subject"] ?? ""; ?></td>
                        <td><?php echo $pdata["expiry_date"] ?? ""; ?></td>
                        <td><?php echo $pdata["total_amount"] ?? ""; ?></td>
                        <td><?php echo $pdata["bill_to_address"] ?? ""; ?></td>
                        <td><?php echo $pdata["warehouse_address"] ?? ""; ?></td>
                        <td><span class="<?php echo $status_class ?? ""; ?>"><?php echo $pdata["stage"] ?? ""; ?></span></td>
                    <?php } else {
                        $status_class = statusClass($pdata["status"]);
                        if ($pdata['status'] == 3) {
                            $pdf_name = $pdata["req_reference_no"] . "_PO_deshwal.pdf";
                            // Generate URL to the controller action
                            $pdf_url = Url::to(['site/downloadpo', 'file' => $pdf_name], true);
                        ...
                        <td><?php echo $pdata["req_reference_no"] ?? ""; ?></td>
                        <td></td>
                        <td><?php echo $pdata["po_expiry_date"] ?? ""; ?></td>
                        <td><?php echo $pdata["total_amount"] ?? ""; ?></td>
                        <td><?php echo $pdata["bill_to_address"] ?? ""; ?></td>
                        <td><?php echo $pdata["warehouse_address"] ?? ""; ?></td>
